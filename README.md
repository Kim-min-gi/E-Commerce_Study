...
- 로그인 및 인증 처리 구현, JWT 기반 무상태 인증 시스템 설계
- Refresh 토큰 관리 및 Access 토큰 블랙리스트 구현으로 보안 강화
...
- CI/CD 자동화 구축 (GitHub Actions + DockerHub + EC2) <br/>
    → 수동 빌드/배포로 인한 시간 낭비 및 실수 발생 <br/>
해결 : GitHub Actions로 빌드 & 푸시 자동화 구성 
수작업 대비 배포 시간 **90% 이상 단축** (약 10분 → 1분 이내)

- 테스트 환경에서 프로퍼티 로딩 실패 <br/>
